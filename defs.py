...
def extract_img():
    ...
    subprocess.run(["./payload-dumper-go", "-c", "8", "-o", "./",
                   "-p", "system,system_ext,product", "payload.bin"])
...
def extract_files():
    try:
        ...
        if "EROFS filesystem" in output:
            ...
            subprocess.run(
                ["./extract.erofs", "-i", "system_ext.img", "-x", "-T16"])
            subprocess.run(
                ["./extract.erofs", "-i", "product.img", "-x", "-T16"])
        elif "data" in output:
...
def delete_files_and_folders():
    ...
    folders_to_delete = ["output_apk", "update_apk",
                         "update_name_apk", "system_ext", "system", "product", "config"]
