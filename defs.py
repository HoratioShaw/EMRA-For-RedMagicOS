            ...
            for line in file:
                if line.startswith("ro.product.system.name"):
                    ...
                    if device_name in is_fold:
                        print("\n检测到包设备为 Fold，请输入-j 0/1(备份/不备份) f 参数切换字库")
                        break
                    elif device_name in is_pad:
                        print("\n检测到包设备为 Pad，请输入-j 0/1(备份/不备份) p 参数切换字库")
                        break
                    else:
                        print("\n检测到包设备为 Phone，请输入-j 0/1(备份/不备份) ph 参数切换字库")
                        break                    
...
def delete_files_and_folders():
    """删除指定的文件和文件夹"""
    files_to_delete = ["payload.bin", "*.img", "app_code_name.json"]
    folders_to_delete = ["output_apk", "update_apk", "update_name_apk", "system_ext", "system", "product", "config"]
